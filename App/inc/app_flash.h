 ...
 * - magic проверяет, что страница содержит конфигурацию;
 * - sensors хранит ROM-коды DS18B20 для логических каналов;
 * - performer_id хранится как 1 байт, потому что CAN NodeID в DDS-240 8-битный;
 * - reserved_bytes явно закрывает выравнивание и будущие расширения;
 * - checksum должен быть последним полем структуры.
 *
 * Размер: 76 байт, кратен 4 байтам для записи во Flash словами uint32_t.
 */
