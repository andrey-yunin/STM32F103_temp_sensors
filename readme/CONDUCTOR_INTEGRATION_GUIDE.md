...
## 1. Сетевые параметры (Transport Layer)
- **Протокол**: bxCAN (STM32), 500 kbps.
- **Формат ID**: 29-bit Extended.
- **NodeID Дирижера**: `0x10`.
- **NodeID Исполнителя насосов (Base)**: `0x30`.
- **Broadcast**: `0x00` (Исполнитель обязан принимать широковещательные команды).

---

## 2. Структура командного фрейма (Payload)
Дирижер упаковывает данные в формате **Little-Endian**. Длина кадра всегда 8 байт.

| Байты | Поле | Тип | Описание |
|:------|:-----|:----|:---------|
| 0-1   | `cmd_code` | uint16 | Код команды (напр. 0x0202). |
| 2     | `device_id`| uint8  | Логический ID устройства (10-25). |
| 3-6   | `timeout`  | uint32 | Таймаут безопасности в мс (0 = Default). |
| 7     | `reserved` | uint8  | Резерв (0x00). |

---

## 3. Реестр логических устройств (Fluidics Device Mapping)
Дирижер должен использовать следующие ID для адресации конкретных каналов платы `0x30`:

### 3.1. Насосы (Pumps 1-13)
- **ID 10-22**: Соответствуют физическим насосам платы.
- **Команды**: `START (0x0202)`, `STOP (0x0203)`, `RUN_DURATION (0x0201)`.

### 3.2. Клапаны (Valves 1-3)
- **ID 23-25**: Соответствуют физическим клапанам платы.
- **Команды**: `OPEN (0x0204)`, `CLOSE (0x0205)`.

---

## 4. Жизненный цикл команды (Transaction)

Дирижер обязан отслеживать состояние транзакции по следующей схеме:

1. **SEND**: Дирижер отправляет команду.
2. **ACK (0x01)**: Дирижер должен получить подтверждение приема в течение **50 мс**. Если `ACK` не получен — узел считается Offline.
3. **DONE (0x03, Sub:0x01)**: Дирижер ожидает сигнал завершения.
   - Для `START/STOP` — в течение **100 мс** после ACK.
   - Для `RUN_DURATION` — в течение **Timeout + 200 мс**.

---

## 5. Требования к валидации на стороне Дирижера

### 5.1. Предотвращение ошибок (Safety First)
- **Type Check**: Дирижеру запрещено отправлять команду `PUMP_START` на `ID 23` (клапан). Исполнитель вернет `NACK (0x0002)`.
- **Zero Duration**: Команда `0x0201` (RUN_DURATION) with `timeout = 0` является невалидной.

### 5.2. Детекция срабатывания Safety Timeout
Если Дирижер получил `ACK`, но **не получил** `DONE` или `STOP` по истечении времени защиты (60с для насосов), он должен:
1. Зафиксировать аварийное отключение устройства по внутреннему таймеру безопасности исполнителя.
2. Обновить статус устройства в UI как "Safety Off / Timeout Interrupted".

### 5.3. Сервисный аудит (Discovery)
...
## 6. Коды ошибок NACK (Error Registry)
| Код | Причина | Действие Дирижера |
|:----|:--------|:------------------|
| `0x0001` | Unknown Command | Проверить версию FW исполнителя. |
| `0x0002` | Invalid Device ID | Проверить маппинг ID (10-25). |
| `0x0004` | Invalid Param/Key | Проверить значение таймаута или Magic Key. |
...
##ок, я конечно за пром стандарт! но уеня все же вопрос       7.5. Жизненный цикл команды (Transaction)
Дирижер обязан отслеживать состояние транзакции по следующей схеме:
